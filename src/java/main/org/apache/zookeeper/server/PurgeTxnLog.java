 ...
 *
 * 在这边简单介绍transaction log 和 snapshot，txnlog是zookeeper的write ahead log，类似数据库中的redolog
 * zk为了提高性能将node state 存储在内存中，因此每一次重启需要从txnlog中恢复状态
 * snapshot则是zk为了提高性能，避免每次都从txnlog中恢复做的某一个时刻的节点快照。
 */
